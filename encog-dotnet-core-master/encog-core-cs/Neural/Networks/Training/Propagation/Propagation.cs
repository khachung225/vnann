    ...
    public abstract class Propagation : BasicTraining, ITrain, IMultiThreadable
    {
        ...
        public override void Iteration()
        {
            try
            {
                ...
                {
                    //cap nhat trong so. tinh toan thep gradient
                    Learn();
                }
        ...
        private void SupperviserWigth()
        {
            try
            {
                var index = GeneraterRandom.GetRandomIndex(0, _flat.Weights.Length-1);
                double value = GeneraterRandom.GetRandomValue();
                _flat.Weights[index] = _flat.Weights[index] + value;
            }
            catch (Exception)
            {
