...
namespace CommodityPredictor.Utilities
{
    /// <summary>
    ...
    public class ErrorCalculation
    {
        ...
        public void UpdateError(double[] actual, IMLData ideal)
        {
            ...
            {
                double delta = ideal[i] - actual[i];
                _globalError += Math.Abs(delta); //delta * delta;
            }
            _setSize += ideal.Count;
        }

        public void UpdateError(double actualClose, double predictedClose)
        {
            double delta = actualClose - predictedClose;
            _globalError += Math.Abs(delta);
            _setSize++;
        }
    }
